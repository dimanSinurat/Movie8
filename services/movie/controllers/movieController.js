const movieModel = require('../models/model_movie');
const {ObjectId} = require('mongodb');
const idFormatedLength = 24;

class Controller {
  static getAll(req, res) {
    movieModel.findMovie()
    .then((data) => {
  ...
  static getOne(req, res) {
    ...
    if (id.length !== idFormatedLength) {
      ...
      movieModel.findOne({_id: ObjectId(id)})
      .then((data) => {
  ...
  static update(req, res) {
    ...
    if (id.length !== idFormatedLength) {
      ...
      if(checkPayload.length === Object.keys(newValue).length && allowedInput){
        movieModel.update({_id : ObjectId(id)}, {$set: newValue})
  ...
  static delete(req, res) {
    ...
    if (id.length !== idFormatedLength) {
      ...
      movieModel.delete({_id: ObjectId(id)})
      .then(({deletedCount}) => {
